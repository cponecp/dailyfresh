...
class OrderPlaceView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        '''提交订单页面显示 前端cart.html用的form提交'''
        # 获取登录的用户
        user = request.user
        # 获取参数sku_ids
        sku_ids = request.POST.getlist('sku_ids') # [1,26] 前端对checkbox中订单中的商品取一样的name = sku_ids，
        ...
        try:
            ...
            for sku_id in sku_ids:
                ...
                try:
                    ...
                    sku = GoodsSKU.objects.select_for_update().get(id=sku_id)  # 上了,其他用户进行这一步查询时,可能 会阻塞在这一步.直到对方commit后,抢到锁
                except:
                    # 商品不存在
...
class OrderCommitView(View):
    ...
    def post(self, request):
        ...
        # 组织参数
        # 订单id: 20171122181630+用户id   用户id最好用id的hash值
        order_id = datetime.now().strftime('%Y%m%d%H%M%S')+str(user.id)
...
class OrderPayView(View):
    ...
    def post(self, request):
        ...
        order_string = alipay.api_alipay_trade_page_pay(
            ...
            total_amount=str(total_pay), # 支付总金额 Decimal类型无法直接序列化，需要转成字符串
